...
class Comment:
    ...
    def __init__(self, column_string):
        ...
        (self._datetime, self._id) = self._parse_others(result[2])
        self._raw_body = self._parse_body(result[3])
    ...
    def _parse_others(self, string):
        result = Comment.__date_id_expressions.search(string)
        if result:
            return (self._parse_datetime(string[:result.start()]),
                    string[result.end():])
        else:
            return (self._parse_datetime(string), "")
